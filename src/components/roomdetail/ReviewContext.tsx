...
type ReviewContextType = {
  isLoading: boolean;
  error: string | null;
  initReviews: (data: roomType, selectedOption: string, page: number) => Promise<void>;
...
export function ReviewProvider({ children }: { children: ReactNode }) {
  ...
  // 초기 리뷰 목록을 불러오는 함수
  const initReviews = useCallback(async (data: roomType, selectedOption: string, page: number) => {
    try {
      setIsLoading(true);
      setError(null);
